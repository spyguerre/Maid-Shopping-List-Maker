...
def generate_ingredients_tab():
    ...
    for recipe, data in RECIPES.items():
        url, ing_start_cell, is_single_piece_recipe = data["url"], data["ing_cell"], data["is_single-piece_recipe"]

        cells1[recipe_index * 22 + 1][0] = f'=HYPERLINK("{url}", "{recipe}")'
        cells1[recipe_index * 22 + 1][1] = f'=IMPORTRANGE("{url}", "RECETTE!L2")' if not is_single_piece_recipe else "/"

        cells1[recipe_index * 22 + 2][0] = "Target pieces:"
        cells1[recipe_index * 22 + 2][1] = "0"

        for i in range(ing_start_cell, 23):  # Assuming max 20 ingredients per recipe
            qty_formula = f'=IMPORTRANGE("{url}", "RECETTE!J{i}")'
            cells1[recipe_index * 22 + i - 1][2] = qty_formula
            mult_qty_formula = f'=IF({get_excel_cell(recipe_index * 22 + i - 1, 4)}=0; \"\"; {get_excel_cell(recipe_index * 22 + i - 1, 2)}*CEILING({get_excel_cell(recipe_index * 22 + 2, 1, True, True)} / {get_excel_cell(recipe_index * 22 + 1, 1, True, True) if not is_single_piece_recipe else 1}))'
